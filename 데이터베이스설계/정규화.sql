...
CREATE TABLE Students (
 ...
 INSERT INTO  students VALUES
    ...
    (2, '임꺽정','국어'),
    (3, '전우치','사회, 국어, 영어');
-- 릴레이션의 속성값은 반드시 원자값이어야 한다. => 1정규화 위배
...
INSERT INTO CourseRegist VALUES
 ...
 -- 삭제이상: 학생1번, 102번 강의 수강 정보를 취소하게 되면, 이영희 강사의 자료구조라는 강의 정보가 사라짐.
 -- 삽입이상: 새로운 강의 104,전우치교수의 컴퓨터도술 강의가 개설되면, 학생번호에 null 값 삽입 문제가 발생
 -- 수정이상: 홍길동 교수가 데이터베이스 강의 대신 안드로이드 강의를 맡게 되면, 데이터 불일치 발생 가능성이 있음.
 
 -- 제 2 정규형으로 테이블을 분해
 -- CourseRegist(StudentId(PK), CourseId(PK), InstructorName, CourseName)
 -- 부분 함수 종속 (CourseId -> InstructorName, CourseName)
 -- 부분 함수 종속성 제거
...
-- 함수 종속성 분석 StudentId(결정자) -> CourseId(종속자) / CourseId(결정자) -> InstructorName(종속자)
-- 이행적 종속 StudentId(결정자) ->  InstructorName(종속자), 학생번호를 알면 강사이름을 알 수 있는 상태
...
CREATE TABLE Instructor (
	CourseId INT PRIMARY KEY,     -- 이행적 종속관계의 결정자를 기본키로 테이블 분해
	INstructorName VARCHAR(255)   -- 종속자
);   
INSERT INTO Instructor (CourseId, INstructorName)
	SELECT CourseId, INstructorName FROM StudentCourse;
    
ALTER TABLE StudentCourse DROP COLUMN InstructorName;  -- 기존 테이블의 컬럼(이행종속자) 삭제

-- 이행적 종속성 제거
-- Before:  StudentId(결정자) -> CourseId(종속자) / CourseId(결정자) -> InstructorName(종속자)
-- StudentCourse (StudentId(PK), CourseId, InstructorName)
-- After
-- StudentId(결정자) -> CourseId(종속자)  //  StudentCourse (StudentId(PK), CourseId )
-- CourseId(결정자) -> InstructorName(종속자) // Instructor (CourseId(PK), InstructorName )

SELECT * FROM Instructor;
SELECT * FROM StudentCourse;

-- 삭제이상 없음: 2번 학생이 수강을 취소하여도, 102번 강의(이강사) 정보는 남아있음
DELETE FROM StudentCourse WHERE StudentId = 2;
-- 삽입이상 없음: 104번 장강사 강의를 개설하더라도, null값 학생번호 삽입하지 않음 
INSERT INTO Instructor VALUES (104, '장강사');
-- 수정이상 없음: 101번 강의를 조강사 맡게 되어도, 데이터 불일치 문제 발생 가능성 없음
...
-- 함수종속성 분석
-- (StudentId,CourseName) (결정자) -> InstructorName (종속자)
-- InstructorName(교수이름, 기본키가 아님) (결정자) -> CourseName (종속자)
-- 모든 결정자가 후보키(기본키가 될 수 있는 속성 집합)

-- 이상현상
-- 삭제이상: 402번 학생이 수강을 취소하면 ' 인간과동물' 특강 정보와 교수 정보가 사라짐
-- 삽입이상: 새로운 특강 정보를 개설해서 최교수가 담당하면, 학생정보 null 값 삽입 문제 발생
-- 수정이상: 김교수의 특강 제목을 '빅터에터분석' 으로 바꾸면 데이터 불일치 가능성 발생
