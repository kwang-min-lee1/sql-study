...
-- 특정 부서(30번)의 평균급여보다 높은 급여를 받는 직원 조회
...
-- 메인쿼리
SELECT e.ename, e.sal, d.dname, d.loc FROM emp e JOIN dept d on e.deptno = d.deptno
...
-- 메인쿼리의 any 함수에 사용 (비교연산자와 함께 사용)
-- 하나라도 큰 것이 있으면 참 => 800보다 많이 받는 모든 직원
SELECT sal FROM emp WHERE sal > ANY(SELECT sal FROM emp WHERE deptno = 20);
...
-- 3000(최대값)보다 큰 경우만 참
SELECT sal FROM emp WHERE sal >= ALL(SELECT sal FROM emp WHERE deptno = 20);
-- 800(최소값) 보다 작은 경우만 참
SELECT sal FROM emp WHERE sal <= ALL(SELECT sal FROM emp WHERE deptno = 20);
...
-- 상호연관 쿼리: 메인쿼리의 칼럼을 서브쿼리에서 사용하는 것
...
SELECT dept_avg.deptno, dept_avg.avg_sal FROM (SELECT deptno, avg(sal) as avg_sal FROM emp GROUP BY deptno) as dept_avg;
