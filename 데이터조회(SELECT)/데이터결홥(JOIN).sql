...
-- 2개의 테이블 조회
SELECT * FROM emp, dept;  -- 모든 가능한 행 14*4 =56의 결과 조회 (카티션 프로젝트, Cartesian Product)

-- 두개의 테이블에서 deptno가 같은 경우만 조회 (WHERE절 2개의 데이클 조건 명시)
SELECT * FROM emp, dept
WHERE emp.deptno = dept.deptno;
-- => 암시적 조인

-- 두개의 테이블을 결합하고, 결과 셋에서 필요한 속성들만 프로젠션(투영)
SELECT ename, job, sal, dname, loc
FROM emp, dept
WHERE emp.deptno = dept.deptno;

-- 양쪽 테이블 모두에 있는 필드이름인 경우, 테이블명을 명시하지 않으면 오류 발생함
SELECT ename, job, sal, dname, loc, dept.deptno
FROM emp, dept
WHERE emp.deptno = dept.deptno;

-- 일반적으로 프로젝션하는 모든 필드의 이름에, 테이블을 함께 명시
SELECT emp.ename, emp.job, emp.sal, dept.dname, dept.loc, dept.deptno
FROM emp, dept
WHERE emp.deptno = dept.deptno;

-- 각 필드가 어떤 테이블에 속했는지 명확해졌지만, 너무 길어진다.
-- 각각의 테이블명세 별칭을 주어, 간결화 시킴 (관례적으로 테이블 앞글자)
SELECT e.ename, e.job, e.sal, d.dname, d.loc, d.deptno
FROM emp e, dept d
WHERE e.deptno = d.deptno;

-- 명시적 조인
-- JOIN 과 ON 키워드를 함꼐 사용
-- 1. Inner Join
SELECT *
FROM emp
...
-- SELF JOIN
-- 별칭을 사용해서, 동일 테이블을 마치 2개의 테이블인것 처럼 
-- 같은 테이블을 2번 참조하여 결합하는 방식
SELECT e1.ename as "직원" , e2.ename "매니져"
FROM emp e1
...
-- 두개의 테이블을 결합할 때 컬럼이 이퀄(=, 등호)가 아닌 비교, 범위 등의 조건으로
-- Join Condition 으로 결합한 경우
-- 직원 테이블의 급여(SAL)가 급여등급 테이블(salgrade)의 losal , hisal범위에 있는 경우로 join
SELECT E.ENAME, E.SAL, S.GRADE
FROM EMP E
JOIN SALEGRADE S ON E.SAL BETWEEN S.LOSAL AND S.HISAL;
...
-- CROSS JOIN
-- 양 테이블의 모든 행을 조인, 결과는 두 테이블을 곱한 갯수, 카디션 프로덕트
-- 많은 데이터 생성시 사용
SELECT * FROM EMP
CROSS JOIN DEPT;
