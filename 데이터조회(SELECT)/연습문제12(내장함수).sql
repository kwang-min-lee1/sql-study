...
SELECT * FROM film;
SELECT title, length, if(length >= 120, '긴 영화', '짤은 영화') as '길이분류'
FROM film;

-- 문제 2: actor 테이블에서 첫 번째 이름(first_name)과 성(last_name)을 합쳐서 전체 이름을 조회하세요.
SELECT concat(first_name,' ', last_name) as '전체 이름'   -- concat_ws(' ', first_name,last_name)
FROM actor;

-- 문제 3: film 테이블에서 영화 제목(title)의 길이를 조회하세요.
SELECT title, length(title) '영화제목길이'
FROM film;

-- 문제 4: rental 테이블에서 각 대여에 대한 대여 시작일(rental_date)과 반납 예정일(return_date) 사이의 일수를 계산하세요.
SELECT rental_date, return_date, datediff(return_date, rental_date) as 대여일수
FROM rental;

-- 문제 5: payment 테이블에서 모든 결제 금액을 달러 자리까지 반올림하여 조회하세요.
SELECT payment_id, round(amount, 0)
FROM payment;
