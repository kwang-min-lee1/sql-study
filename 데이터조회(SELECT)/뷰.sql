...
-- 기존의 동일 이름의 뷰가 있으면 오류 발생시키지 않고, 덮어쓰기
...
-- 뷰를 통해서 데이터를 입력하기
-- 원본 테이블의 제약조건(birthYear 에서 Not Null)을 만족시킬 수 없기 때문에
...
-- 원본 데이터에는 삽입이 되었음 => (뷰에서는 조회, 수정, 삭제 불가)
SELECT * FROM usertbl;
...
ALTER VIEW view_height_upper177 AS
	SELECT * FROM usertbl WHERE heigth >= 177
		WITH CHECK OPTION;
        
 -- 뷰를 통해 데이터 삽입 =>CHECK OPTION 으로 데이터 삽입 실패 =? 의도하지 않은 경로 삽입을 막음 => 무결성 지킴
 INSERT INTO view_height_upper177
 VALUES ('SDR', '숏다리2', 1990, '부산', '010', '22222222', 140, '2024-01-01');
