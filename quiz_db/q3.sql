...
DELIMITER $$
CREATE PROCEDURE GetEmployeeSalary (IN emp_no INT, IN year_val INT)
BEGIN
	SELECT salaries, from_date, to_date
	FROM salaries
    WHERE emp_no = emp_no
		AND YEAR(from_date) = year_val;
END$$
DELIMITER ;

CALL GetEmployeeSalary (10001, 2000);